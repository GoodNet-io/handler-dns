...
MemoryDnsBackend::MemoryDnsBackend()
    : clock_(&monotonic_default_clock_us) {}

MemoryDnsBackend::MemoryDnsBackend(std::uint64_t (*clock)() noexcept)
    : clock_(clock != nullptr ? clock : &monotonic_default_clock_us) {}

bool MemoryDnsBackend::put(std::string_view key,
                      std::span<const std::uint8_t> value,
                      std::uint64_t ttl_s,
                      std::uint8_t flags) {
    if (key.empty() || key.size() > GN_DNS_KEY_MAX_LEN) return false;
    if (value.size() > GN_DNS_VALUE_MAX_LEN)            return false;
    Entry e{
        .key          = std::string{key},
...
std::optional<Entry> MemoryDnsBackend::get(std::string_view key) const {
...
std::vector<Entry>
MemoryDnsBackend::get_prefix(std::string_view prefix, std::uint32_t max_results) const {
...
std::vector<Entry>
MemoryDnsBackend::get_since(std::uint64_t since_us, std::uint32_t max_results) const {
    std::vector<Entry> out;
    out.reserve(std::min<std::size_t>(max_results, map_.size()));
...
bool MemoryDnsBackend::del(std::string_view key) {
    return map_.erase(std::string{key}) > 0;
}

std::uint64_t MemoryDnsBackend::cleanup_expired(std::uint64_t now_us) {
...
/// Build a DNS_RESULT envelope. `entries` is empty for PUT/DELETE
...
std::optional<PutView> parse_put(std::span<const std::uint8_t> src) {
    ...
    if (kl == 0 || kl > GN_DNS_KEY_MAX_LEN)   return std::nullopt;
    if (vl > GN_DNS_VALUE_MAX_LEN)            return std::nullopt;
...
std::optional<GetView> parse_get(std::span<const std::uint8_t> src) {
    ...
    v.request_id  = read_be<std::uint64_t>(src.subspan(0, 8));
    const auto m  = src[8];
    if (m > GN_DNS_QUERY_SINCE) return std::nullopt;
    v.mode        = static_cast<gn_dns_query_t>(m);
    ...
    if (kl > GN_DNS_KEY_MAX_LEN) return std::nullopt;
...
parse_subscribe(std::span<const std::uint8_t> src) {
    ...
    v.request_id  = read_be<std::uint64_t>(src.subspan(0, 8));
    const auto m  = src[8];
    if (m != GN_DNS_QUERY_EXACT && m != GN_DNS_QUERY_PREFIX) {
        return std::nullopt;
    }
    v.mode        = static_cast<gn_dns_query_t>(m);
...
DnsHandler::DnsHandler(const host_api_t* api)
    : DnsHandler(api, std::make_unique<MemoryDnsBackend>(), &default_clock_us) {}
      ...
      now_us_(clock)
{
    ...
    vtable_.protocol_id        = &DnsHandler::vtable_protocol_id;
    vtable_.supported_msg_ids  = &DnsHandler::vtable_supported_msg_ids;
    vtable_.handle_message     = &DnsHandler::vtable_handle_message;

    ext_vtable_.api_size          = sizeof(gn_dns_api_t);
    ext_vtable_.put               = &DnsHandler::ext_put;
    ext_vtable_.get               = &DnsHandler::ext_get;
    ext_vtable_.query             = &DnsHandler::ext_query;
    ext_vtable_.del               = &DnsHandler::ext_del;
    ext_vtable_.subscribe         = &DnsHandler::ext_subscribe;
    ext_vtable_.unsubscribe       = &DnsHandler::ext_unsubscribe;
    ext_vtable_.cleanup_expired   = &DnsHandler::ext_cleanup_expired;
...
gn_propagation_t DnsHandler::handle_message(const gn_message_t* env) {
    ...
    case kMsgPut: {
        ...
        if (ok) notify(e_view, GN_DNS_EVENT_PUT);
        return GN_PROPAGATION_CONSUMED;
    }
    ...
    case kMsgGet: {
        ...
        std::vector<Entry> hits;
        {
            std::lock_guard lk(mu_);
            switch (v->mode) {
            case GN_DNS_QUERY_EXACT:
                if (auto e = backend_->get(v->key)) hits.push_back(*e);
                break;
            case GN_DNS_QUERY_PREFIX:
                hits = backend_->get_prefix(v->key,
                    std::min(v->max_results, GN_DNS_QUERY_MAX_RESULTS));
                break;
            case GN_DNS_QUERY_SINCE:
    ...
    case kMsgDelete: {
        ...
        if (removed && snap) notify(*snap, GN_DNS_EVENT_DELETE);
        return GN_PROPAGATION_CONSUMED;
    }
...
void DnsHandler::send_result(gn_conn_id_t conn,
                                 std::uint64_t request_id,
                                 std::uint8_t status,
                                 const std::vector<Entry>& entries) {
...
void DnsHandler::notify(const Entry& entry, gn_dns_event_t event) {
...
int DnsHandler::put_local(std::string_view key,
                             std::span<const std::uint8_t> value,
                             std::uint64_t ttl_s, std::uint8_t flags) {
    if (key.empty() || key.size() > GN_DNS_KEY_MAX_LEN) return -1;
    if (value.size() > GN_DNS_VALUE_MAX_LEN)            return -1;
    bool ok;
    {
        std::lock_guard lk(mu_);
    ...
    notify(snap, GN_DNS_EVENT_PUT);
    return 0;
}

std::optional<Entry> DnsHandler::get_local(std::string_view key) const {
    std::lock_guard lk(mu_);
    return backend_->get(key);
}

std::vector<Entry>
DnsHandler::query_local(gn_dns_query_t mode, std::string_view key,
                            std::uint64_t since_us,
                            std::uint32_t max_results) const {
    std::lock_guard lk(mu_);
    const auto cap = std::min(max_results, GN_DNS_QUERY_MAX_RESULTS);
    switch (mode) {
    case GN_DNS_QUERY_EXACT:
        if (auto e = backend_->get(key)) return {*e};
        return {};
    case GN_DNS_QUERY_PREFIX: return backend_->get_prefix(key, cap);
    case GN_DNS_QUERY_SINCE:  return backend_->get_since(since_us, cap);
    }
    return {};
}

bool DnsHandler::del_local(std::string_view key) {
    std::optional<Entry> snap;
    bool removed;
    {
        std::lock_guard lk(mu_);
...
std::uint64_t DnsHandler::subscribe_local(gn_dns_query_t mode,
                                              std::string_view key,
                                              gn_dns_event_cb_t cb,
                                              void* user_data) {
...
void DnsHandler::unsubscribe_local(std::uint64_t token) noexcept {
    if (token == 0) return;
    std::lock_guard lk(mu_);
...
std::uint64_t DnsHandler::cleanup_expired_local() {
    std::lock_guard lk(mu_);
...
const char* DnsHandler::vtable_protocol_id(void* /*self*/) {
    return kProtocolId;
}

void DnsHandler::vtable_supported_msg_ids(void* /*self*/,
...
gn_propagation_t
DnsHandler::vtable_handle_message(void* self, const gn_message_t* env) {
    return static_cast<DnsHandler*>(self)->handle_message(env);
}

// ── extension thunks ─────────────────────────────────────────────────────

int DnsHandler::ext_put(void* ctx, const char* k, size_t kl,
                            const std::uint8_t* v, size_t vl,
                            std::uint64_t ttl, std::uint8_t flags) {
    if (k == nullptr) return -1;
    return static_cast<DnsHandler*>(ctx)->put_local(
        {k, kl}, {v, vl}, ttl, flags);
}

int DnsHandler::ext_get(void* ctx, const char* k, size_t kl,
                            gn_dns_record_t* out) {
...
int DnsHandler::ext_query(void* ctx, gn_dns_query_t mode,
...
int DnsHandler::ext_del(void* ctx, const char* k, size_t kl) {
    if (k == nullptr) return -2;
    return static_cast<DnsHandler*>(ctx)->del_local({k, kl}) ? 0 : -1;
}

uint64_t DnsHandler::ext_subscribe(void* ctx, gn_dns_query_t mode,
                                       const char* k, size_t kl,
                                       gn_dns_event_cb_t cb, void* user) {
    if (k == nullptr || cb == nullptr) return 0;
    return static_cast<DnsHandler*>(ctx)->subscribe_local(
        mode, {k, kl}, cb, user);
}

void DnsHandler::ext_unsubscribe(void* ctx, std::uint64_t token) {
    static_cast<DnsHandler*>(ctx)->unsubscribe_local(token);
}

uint64_t DnsHandler::ext_cleanup_expired(void* ctx) {
    return static_cast<DnsHandler*>(ctx)->cleanup_expired_local();
}
