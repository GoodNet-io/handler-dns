...
/// @file   plugins/handlers/dns/sqlite_backend.hpp
/// @brief  File-backed IDnsBackend using SQLite. Slice 2.
///
...
/// SQLite-backed IDnsBackend. Single connection, single mutex — the
...
class SqliteDnsBackend final : public IDnsBackend {
public:
    ...
    explicit SqliteDnsBackend(const std::string& db_path);

    /// Optional clock injection for tests. Production callers omit
    /// it and get the monotonic wall-clock wrapper.
    SqliteDnsBackend(const std::string& db_path,
                std::uint64_t (*clock)() noexcept);

    ~SqliteDnsBackend() override;

    SqliteDnsBackend(const SqliteDnsBackend&)            = delete;
    SqliteDnsBackend& operator=(const SqliteDnsBackend&) = delete;
