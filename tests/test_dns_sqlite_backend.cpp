...
/// @file   plugins/handlers/dns/tests/test_dns_sqlite_backend.cpp
/// @brief  SqliteDnsBackend — backend contract round-trips on the same
///         test matrix that pins MemoryDnsBackend (slice 1 reference).
///         A handler driven through SqliteDnsBackend must observe the
...
TEST(SqliteDnsBackend_PutGet, RoundtripsValueInMemory) {
    SqliteDnsBackend s(":memory:");
...
TEST(SqliteDnsBackend_PutGet, RejectsEmptyKey) {
    SqliteDnsBackend s(":memory:");
    EXPECT_FALSE(s.put("", {}, 0, 0));
    EXPECT_EQ(s.size(), 0u);
}

TEST(SqliteDnsBackend_PutGet, RejectsOversizedKey) {
    SqliteDnsBackend s(":memory:");
    std::string huge(GN_DNS_KEY_MAX_LEN + 1, 'x');
    EXPECT_FALSE(s.put(huge, {}, 0, 0));
}

TEST(SqliteDnsBackend_PutGet, RejectsOversizedValue) {
    SqliteDnsBackend s(":memory:");
    std::vector<std::uint8_t> huge(GN_DNS_VALUE_MAX_LEN + 1);
    EXPECT_FALSE(s.put("k", huge, 0, 0));
}

TEST(SqliteDnsBackend_PutGet, MissReturnsNullopt) {
    SqliteDnsBackend s(":memory:");
    EXPECT_FALSE(s.get("missing").has_value());
}

TEST(SqliteDnsBackend_PutGet, OverwriteKeepsLatestValue) {
    SqliteDnsBackend s(":memory:");
...
TEST(SqliteDnsBackend_PutGet, PreservesBinaryKey) {
    SqliteDnsBackend s(":memory:");
...
TEST(SqliteDnsBackend_Prefix, MatchesByPrefix) {
    SqliteDnsBackend s(":memory:");
...
TEST(SqliteDnsBackend_Prefix, EmptyPrefixReturnsAll) {
    SqliteDnsBackend s(":memory:");
...
TEST(SqliteDnsBackend_Prefix, RespectsMaxResults) {
    SqliteDnsBackend s(":memory:");
    for (int i = 0; i < 10; ++i) {
...
TEST(SqliteDnsBackend_Delete, RemovesEntry) {
    SqliteDnsBackend s(":memory:");
    ASSERT_TRUE(s.put("k", std::vector<std::uint8_t>{1}, 0, 0));
    EXPECT_TRUE(s.del("k"));
...
TEST(SqliteDnsBackend_Delete, MissReturnsFalse) {
    SqliteDnsBackend s(":memory:");
    EXPECT_FALSE(s.del("nope"));
}

TEST(SqliteDnsBackend_Cleanup, DropsExpired) {
    SqliteDnsBackend s(":memory:");
...
TEST(SqliteDnsBackend_Since, FiltersByTimestamp) {
    SqliteDnsBackend s(":memory:");
    ASSERT_TRUE(s.put("a", std::vector<std::uint8_t>{1}, 0, 0));
...
TEST(SqliteDnsBackend_Persistence, RoundtripsThroughFile) {
...
TEST(SqliteDnsBackend_Constructor, RejectsInvalidPath) {
    ...
    EXPECT_THROW(
        SqliteDnsBackend("/this/path/should/never/exist/store.sqlite3"),
        std::runtime_error);
}
