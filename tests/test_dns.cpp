...
/// @brief  DnsHandler — memory backend semantics + wire dispatch +
...
/// Build a DnsHandler with a memory backend + mock clock.
auto make_handler(const host_api_t* api) {
    clock_set(1'000'000);  // reset for each test
    return std::make_unique<DnsHandler>(
        api, std::make_unique<MemoryDnsBackend>(), &mock_clock);
}

// ── MemoryDnsBackend behaviour ────────────────────────────────────────────────

TEST(MemoryDnsBackend_PutGet, RoundtripsValue) {
    MemoryDnsBackend s;
...
TEST(MemoryDnsBackend_PutGet, RejectsEmptyKey) {
    MemoryDnsBackend s;
    EXPECT_FALSE(s.put("", {}, 0, 0));
}

TEST(MemoryDnsBackend_PutGet, RejectsOversizedKey) {
    MemoryDnsBackend s;
    std::string huge(GN_DNS_KEY_MAX_LEN + 1, 'x');
    EXPECT_FALSE(s.put(huge, {}, 0, 0));
}

TEST(MemoryDnsBackend_PutGet, RejectsOversizedValue) {
    MemoryDnsBackend s;
    std::vector<std::uint8_t> huge(GN_DNS_VALUE_MAX_LEN + 1);
    EXPECT_FALSE(s.put("k", huge, 0, 0));
}

TEST(MemoryDnsBackend_PutGet, MissReturnsNullopt) {
    MemoryDnsBackend s;
    EXPECT_FALSE(s.get("missing").has_value());
}

TEST(MemoryDnsBackend_PutGet, OverwriteKeepsLatestValue) {
    MemoryDnsBackend s;
...
TEST(MemoryDnsBackend_Prefix, MatchesByPrefix) {
    MemoryDnsBackend s;
...
TEST(MemoryDnsBackend_Prefix, EmptyPrefixReturnsAll) {
    MemoryDnsBackend s;
...
TEST(MemoryDnsBackend_Prefix, RespectsMaxResults) {
    MemoryDnsBackend s;
    for (int i = 0; i < 10; ++i) {
...
TEST(MemoryDnsBackend_Delete, RemovesEntry) {
    MemoryDnsBackend s;
    ASSERT_TRUE(s.put("k", std::vector<std::uint8_t>{1}, 0, 0));
    EXPECT_TRUE(s.del("k"));
...
TEST(MemoryDnsBackend_Delete, MissReturnsFalse) {
    MemoryDnsBackend s;
    EXPECT_FALSE(s.del("nope"));
}

TEST(MemoryDnsBackend_Cleanup, DropsExpired) {
    MemoryDnsBackend s;
    clock_set(1'000'000);  // = 1s
    /// Inserting through MemoryDnsBackend::put stamps the entry with the
...
TEST(MemoryDnsBackend_Since, FiltersByTimestamp) {
    MemoryDnsBackend s;
    ASSERT_TRUE(s.put("a", std::vector<std::uint8_t>{1}, 0, 0));
...
TEST(StoreExtension, SubscribeFiresOnPut) {
    ...
    struct Capture {
        gn_dns_event_t last_event{};
        std::string      last_key;
        bool             called = false;
    } cap;
    const auto tok = h->subscribe_local(
        GN_DNS_QUERY_PREFIX, "peer/",
        [](void* u, gn_dns_event_t ev, const gn_dns_record_t* e) {
    ...
    EXPECT_EQ(cap.last_event, GN_DNS_EVENT_PUT);
...
TEST(StoreExtension, SubscribeFiresOnDelete) {
    ...
    struct Capture { gn_dns_event_t ev{}; bool called = false; } cap;
    (void)h->subscribe_local(GN_DNS_QUERY_EXACT, "svc/chat",
        [](void* u, gn_dns_event_t e, const gn_dns_record_t*) {
            auto* c = static_cast<Capture*>(u);
            c->ev = e;
            c->called = true;
        }, &cap);
    ...
    EXPECT_TRUE(cap.called);
    EXPECT_EQ(cap.ev, GN_DNS_EVENT_DELETE);
}
...
TEST(StoreExtension, UnsubscribeStopsNotifications) {
    ...
    const auto tok = h->subscribe_local(
        GN_DNS_QUERY_PREFIX, "p/",
        [](void* u, gn_dns_event_t, const gn_dns_record_t*) {
...
namespace wire {
    /// Mirror of the framing helpers in dns.cpp — duplicated here
    ...
    std::vector<std::uint8_t>
    get(std::uint64_t req, gn_dns_query_t mode, std::uint32_t max,
...
TEST(StoreWire, PutFollowedByGetReturnsValue) {
    ...
    /// One DNS_RESULT envelope on the wire (the ACK).
...
TEST(StoreWire, GetMissEmitsNotFoundStatus) {
    ...
    const auto get_bytes = wire::get(11, GN_DNS_QUERY_EXACT, 0, 0, "missing");
    auto env = make_env(kMsgGet, 99, get_bytes);
    EXPECT_EQ(h->handle_message(&env), GN_PROPAGATION_CONSUMED);
    EXPECT_EQ(host.send_calls.load(), 1);
    /// Status byte sits at offset 8 of the DNS_RESULT envelope.
    {
        std::lock_guard lk(host.mu);
...
TEST(StoreWire, SubscribeOverWireFiresNotifyOnPut) {
    ...
    sub[8] = static_cast<std::uint8_t>(GN_DNS_QUERY_PREFIX);
    ...
    /// a DNS_NOTIFY on conn 42 even though it wasn't the writer.
