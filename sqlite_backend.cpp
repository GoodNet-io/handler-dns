...
SqliteDnsBackend::SqliteDnsBackend(const std::string& db_path)
    : SqliteDnsBackend(db_path, &monotonic_default_clock_us) {}
...
SqliteDnsBackend::SqliteDnsBackend(const std::string& db_path,
    ...
    if (sqlite3_open(db_path.c_str(), &db_) != SQLITE_OK) {
        ...
        db_ = nullptr;
        throw std::runtime_error("SqliteDnsBackend: " + err);
    }
    char* err = nullptr;
    if (sqlite3_exec(db_, kPragmas, nullptr, nullptr, &err) != SQLITE_OK) {
        ...
        sqlite3_free(err);
        sqlite3_close(db_);
        throw std::runtime_error("SqliteDnsBackend: " + msg);
    }
    create_schema();
    prepare_statements();
}

SqliteDnsBackend::~SqliteDnsBackend() {
    finalize_statements();
    if (db_) (void)sqlite3_close(db_);
}

void SqliteDnsBackend::create_schema() {
    char* err = nullptr;
...
void SqliteDnsBackend::prepare_statements() {
    ...
    auto prep = [this](sqlite3_stmt*& slot, const char* sql) {
        if (sqlite3_prepare_v2(db_, sql, -1, &slot, nullptr) != SQLITE_OK) {
            throw std::runtime_error(
                std::string{"SqliteDnsBackend: prepare failed: "}
                + sqlite3_errmsg(db_));
        }
    };
...
void SqliteDnsBackend::finalize_statements() noexcept {
    sqlite3_stmt** slots[] = {
...
Entry SqliteDnsBackend::row_to_entry(sqlite3_stmt* stmt) const {
    Entry e;
...
bool SqliteDnsBackend::put(std::string_view key,
...
std::optional<Entry> SqliteDnsBackend::get(std::string_view key) const {
    std::lock_guard lk(mu_);
...
std::vector<Entry>
SqliteDnsBackend::get_prefix(std::string_view prefix,
...
std::vector<Entry>
SqliteDnsBackend::get_since(std::uint64_t since_us,
                         std::uint32_t max_results) const {
    std::vector<Entry> out;
    const auto cap = std::min(max_results, GN_DNS_QUERY_MAX_RESULTS);
    std::lock_guard lk(mu_);
...
bool SqliteDnsBackend::del(std::string_view key) {
    std::lock_guard lk(mu_);
...
std::uint64_t SqliteDnsBackend::cleanup_expired(std::uint64_t now_us) {
    std::lock_guard lk(mu_);
...
std::size_t SqliteDnsBackend::size() const {
    std::lock_guard lk(mu_);
